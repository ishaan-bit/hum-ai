...
`signal-lab axes` trains the coarse valence/arousal LogRegs from RAVDESS features and stages them to the client. Honest accuracy + gate status ride in `model_manifest.json`: arousal cleared the experimental 80 % gate (≈83 %), valence did not (≈69 %, "developing"). Any **mel-CNN affect model is Python-only and not browser-runnable** (no mel-CNN export path), and **none has been promoted** (see the status correction below), so the browser valence axis is the below-gate feature model, surfaced as "developing". Capture also disables browser noise-suppression/AGC (which was attenuating sustained hums) and shows a live level/pitch meter.
