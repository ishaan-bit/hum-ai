...
- **Final commit:** `191b00a` (`feat(stable-build-v13): unsupervised within-hum chunking …`), branch `main`. This hash line is finalized in the immediately-following docs commit, per the v8–v12 convention.
- **Verified production deploy:** `hum-ai-beige.vercel.app` → HTTP 200, serving build asset `index-gqgQXHwp.js` (matches the clean `191b00a` worktree build) with the `within-hum-card` element present and the v13 surface strings ("Across this hum", the musical/inner-state chunk tag, "your usual range") live in the bundle; Vercel CLI prebuilt deploy → production deployment `hum-mewji06z9-ishaans-projects-f5eaf242.vercel.app`. Deployed from an isolated worktree of `191b00a` so the unrelated in-progress sound-lab work in the tree was NOT shipped.
